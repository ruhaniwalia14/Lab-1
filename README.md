...
Can do so by initializing the counter variable outside the loop
and manually updating it inside.
...
2. `git commit -m "refactored to extract helper method"`
   ...
   - If run `git log` will need to use `q` for quit to exit and return to terminal
